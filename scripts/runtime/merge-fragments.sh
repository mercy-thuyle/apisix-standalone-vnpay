...
#
# ── THỨ TỰ OUTPUT ────────────────────────────────────────────────────────────
#   upstreams → services → plugin_configs → routes → global_rules → consumer_groups → consumers → ssls → #END
...
VALID_KEYS="upstreams services plugin_configs routes global_rules consumer_groups consumers ssls"
...
# Thứ tự theo chiều phụ thuộc: upstream → service → plugin_config → route → global rule → consumer group → consumer → ssl
...
log_info "  upstreams=${U}  services=${SVC} plugin_configs=${PC} routes=${R}  global_rules=${GR}  consumer_groups=${CG}  consumers=${CON}  ssls=${S}"
