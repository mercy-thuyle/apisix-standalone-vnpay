...
echo "▶  Verify PASTE_CONTENT còn sót lại, output phải là 0:\n▶  grep "PASTE_CONTENT" apisix_routes/apisix-${DC_PROFILE}.yaml | wc -l  # phải là 0"
echo "▶  docker exec apisix-standalone grep -c "PASTE_CONTENT" /usr/local/apisix/conf/apisix-${DC_PROFILE}.yaml"
echo "▶  Check inode trên host:\n▶  stat apisix_routes/apisix-${DC_PROFILE}.yaml | grep Inode"
echo "▶  Check inode trong container:\n▶  docker exec apisix-standalone stat /usr/local/apisix/conf/apisix-${DC_PROFILE}.yaml | grep Inode"
