...
#
# Usage:
#   merge-routes.sh <routes_src_dir> <output_file>
# Được gọi bởi: gitsync.sh
# =============================================================================
...
if [ -z "${DC_PROFILE:-}" ]; then
  echo "[merge-routes] ERROR: DC_PROFILE chưa được set" >&2
  exit 1
fi
...
for d in upstreams routes ssls; do
  if [ ! -d "${ROUTES_SRC}/${d}" ]; then
    echo "[merge-routes] ERROR: Thiếu thư mục ${ROUTES_SRC}/${d}" >&2
    exit 1
  fi
done
...
ERROR_FLAG="/tmp/merge-routes-error.$$"   # tồn tại = có hard error
WARN_FILE="/tmp/merge-routes-warn.$$"     # đếm warnings
...
# ── Helpers ───────────────────────────────────────────────────────────────────
log_info()  { echo "[merge-routes] $*"; }

log_warn() {
  echo "[merge-routes] WARN: $*" >&2
  echo "1" >> "${WARN_FILE}"
}

log_error() {
  echo "[merge-routes] ERROR: $*" >&2
...
if [ -f "${ERROR_FLAG}" ]; then
  echo "[merge-routes] ABORT: Có lỗi cấu trúc — không ghi output. Sửa lỗi và commit lại." >&2
  exit 1
fi
...
cat > "${TMP_OUTPUT}" << EOF
# apisix-${DC_PROFILE}.yaml — AUTO-GENERATED by merge-routes.sh
